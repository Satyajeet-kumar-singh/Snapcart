...
function AdminDashboardClient({earning,stats,chartData}:propType) {
    ...
    const title = filter === "today" ? "Today's Earning" : filter === "sevenDays" ? "Last 7 Days Earning" : "Total"
  return (
    <div className='pt-28 w-[90%] md:w-[80%] mx-auto'>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10 text-center sm:text-left'>
        ...
        className='text-3xl md:text-xl font-bold text-green-700'>
           🏘️ Admin Dashboard
        </motion.h1>
      ...
      className='bg-green-50 border-green-200 shadow-sm rounded-2xl p-6 text-center mb-10'>
        ...
        <p className='text-4xl font-semibold text-green-800'>₹{currentEarning.toLocaleString()}</p>
      </motion.div>
        ...
        <h2 className='text-lg font-semibold text-gray-700 mb-4'>📊 Orders Overview ( Last 7 Days )</h2>
         <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData}>
              <CartesianGrid stroke="accc" strokeDasharray="5 5"/>
              <XAxis dataKey="day"/>
              <Tooltip/>
