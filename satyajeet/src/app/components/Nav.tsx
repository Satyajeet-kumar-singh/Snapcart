...
//type define kiya destructure k sth
function Nav({user}:{user:IUser}) {
    ...
    <AnimatePresence>
      ...
      className='fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-999 bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white'>
        ...
        <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner '>
          <div className='flex items-center justify-center relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg '>
            {user.image ? <Image src={user.image} fill alt='user' className='object-cover rounded-full'/> : <User/> }
          </div>
          <div>
        ...
        <div onClick={()=>{signOut({callbackUrl:"/login"})}} className='flex items-center gap-3 text-red-300 font-semibold mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all'>
  ...
  return (
    <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
      ...
      {
        user.role == "user" &&
      <form onSubmit={(e)=>{handleSearch(e)}} className='hidden md:flex items-center bg-white rounded-full px-4 py-2 w-1/2 max-w-lg shadow-md'>
        <Search className="text-gray-500 w-5 h-5 mr-2"/>
        <input type="text" value={search} onChange={(e)=>{setSearch(e.target.value)}} placeholder='Search groceries...' className="w-full outline-none text-gray-700 placeholder-gray-400"/>
      </form>
      }
      ...
      <div className='flex items-center gap-3 md:gap-6 relative'>
...

        {
          ...
          <>
            ...
            <div onClick={()=>{setMenuOpen(prev=>!prev)}} className='md:hidden bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md'>
        ...
        <div onClick={()=>{setOpen(prev=>!prev)}} className='bg-white rounded-full w-11 h-11 flex items-center justify-center overflow-hidden shadow-md hover:scale-105 transition-transform'>
          {
        ...
        <AnimatePresence>
          {
            ...
            className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-xl border border-gray-200 p-3 z-999">
              <div className='flex items-center gap-3 px-3 py-2 border-b border-gray-200'>
                <div className='w-10 h-10 rounded-full bg-green-100 flex justify-center items-center overflow-hidden'>
                  {user.image ? <Image src={user.image} fill alt='user' className='object-cover rounded-full'/> : <User/>}
                </div>
                <div>
              ...
              {
                user.role == "user" &&
              <Link href={"/user/my-orders"} onClick={()=>{setOpen(false)}} className='flex items-center gap-2 px-3 py-3 hover:bg-green-50 rounded-lg text-gray-500 font-medium'>
                <Package className='w-5 h-5 text-green-600'/>
                My Orders
              </Link>
              }
              <button onClick={()=>{setOpen(false); signOut({callbackUrl:"/login"})}}  className="flex items-center gap-2 w-full text-left px-3 py-3 hover:bg-red-50 rounded-lg text-gray-700 font-medium">
        ...
        {/* small device k liye search bar */}
        <AnimatePresence>
          {
            searchBarOpen 
            &&
            ...
            className='fixed top-24 left-1/2 -translate-x-1/2 w-[90%] bg-white rounded-full shadow-lg z-40 flex items-center px-4 py-2'>
              ...
              <form onSubmit={(e)=>{handleSearch(e)}} className='grow'>
                <input type="text" value={search} onChange={(e)=>{setSearch(e.target.value)}} placeholder='Search groceries...' className='w-full outline-none text-gray-700'/>
              </form>
