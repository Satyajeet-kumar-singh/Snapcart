...
async function AdminDashboard() {
  ...
  const startOfTheDay = new Date(today)
  startOfTheDay.setHours(0,0,0,0)

  const sevenDaysAgo = new Date()
  sevenDaysAgo.setDate(today.getDate() - 6)

  const todayOrders = orders.filter((o)=> new Date(o.createdAt) >= startOfTheDay)
  ...
  const stats = [
    ...
    {title:"Total Custimers",value:totalCustomers},
  ...
  for(let i=6;i>=0;i--){
    ...
    chartData.push({day:date.toLocaleDateString("en-US",{weekday:"short"}),
      orders:ordersCount
    })
  }
