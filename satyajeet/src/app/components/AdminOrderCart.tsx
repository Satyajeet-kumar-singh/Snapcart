...
function AdminOrderCart({order}:{order:IOrder}) {
    ...
    const updateStatus=async(orderId:string,status:string)=>{
      ...
      try {
        ...
        console.log("UPDATE_STATUS",result.data)
        setStatus(status)
      } catch (error) {
        console.log(error)
      }
    ...
    className='bg-white shadow-md hover:shadow-lg border border-gray-100 rounded-2xl p-6 transition-all'>
      <div className='flex flex-col md:flex-row md:items-start md:justify-between gap-4'>
        <div className='space-y-1'>
            ...
            { status !== "delivered" &&
            <span className={`inline-block text-xs font-semibold px-3 py-1 rounded-full border ${order.isPaid ? "bg-gray-100 text-green-700 border-green-300" : "bg-red-100 textred-700 border-red-300"}`}>
        ...
        <div className='flex flex-col items-start md:items-end gap-2'>
            <span className={`text-xs font-semibold px- py-1 rounded-full capitalize ${status === "delivered" ? "bg-green-100 text-green-700" : status == "pending" ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}>
                {status}
            </span>
            { status !== "delivered" &&
            <select value={status} onChange={(e)=>{updateStatus(order._id?.toString()!,e.target.value)}} className='border border-gray rounded-lg px-3 py-1 text-sm shadow-sm hover:border-green-400 transition focus:ring-2 focus:ring-green-500 outline-none'>
                    ...
                    {order.items.map((item,index)=>(
                      ...
                      className='flex justify-between items-center bg-gray-50 rounded-xl px-3 py-2 hover:bg-gray-100 transition'>
                        <div className='flex items-center gap-3'>
                            <Image src={item.image} width={48} height={48} alt="items" className='rounded-lg object-cover border border-gray-200'/>
                          <div>
