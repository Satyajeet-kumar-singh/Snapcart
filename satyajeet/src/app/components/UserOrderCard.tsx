...
function UserOrderCard({order}:{order:IOrder}) {
  ...
  const getStatusColor=(status:string)=>{
    switch (status) {
      ...
      case "delivered":
        return "bg-green-100 text-green-700 border-green-3000"
      default:
  ...
  return (
    ...
    className='bg-white rounded-2xl border border-gray-100 shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden mt-5'>
      <div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 bg-linear-to-r from-green-50 to-white'>
        <div>
          <h3 className='text-lg font-semibold text-gray-800'>order <span className='text-green-700 font-bold'>
            {order?._id?.toString()?.slice(-6)}</span></h3>
      ...
      <div className='p-5 space-y-4'>
        ...
        {order.assignedDeliveryBoy && (
          ...
          <button onClick={()=>{router.push(`/user/track-order/${order._id?.toString()}`)}} className='w-full flex items-center justify-center gap-2 bg-green-600 text-white font-semibold px-4 py-2 rounded-xl shadow hover:bg-green-700 transition'>
            <Truck size={18}/> 
            Track Your Order
          </button>
          </>
